...
import { useLazySendMessageQuery } from "@/redux/services/chatApi";
...
export default function ChatBot() {
  ...
  const [triggerSendMessage, { data, error, isLoading }] =
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      // Call chatbot API
      const { answer, responseList } = await triggerSendMessage({
        message: userMessage.text,
      }).unwrap();
      // Expected format: { answer: string, responseList: [{ id, name, indexImage, price }] }
      console.log("Chatbot response:", { answer, responseList });
      const botMessage: Message = {
        id: Date.now() + 1,
        text: answer || "Xin lỗi, tôi không hiểu câu hỏi của bạn.",
        sender: "bot",
        timestamp: new Date(),
        products: responseList || [], // Nếu responseList rỗng hoặc undefined thì chỉ hiện text
      };
