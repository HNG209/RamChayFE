...
export default function UpdateProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Kiểm tra sơ bộ số liệu để convert
      const priceVal = formData.price ? parseFloat(formData.price) : null;
      const stockVal = formData.stock ? parseInt(formData.stock) : null;
      const catVal = formData.categoryId ? Number(formData.categoryId) : null;
      ...
      if (error?.data) {
        ...
        const message = error.data.message || JSON.stringify(error.data);
        msg = `${code}${message}`;
        ...
        {errorMessage && (
          <div className="mb-6 bg-red-50 border border-red-200 rounded-xl p-4 flex items-start gap-3 animate-in fade-in slide-in-from-top-2">
            ...
            <div className="flex-1">
              <h3 className="text-sm font-bold text-red-800">Cập nhật thất bại</h3>
